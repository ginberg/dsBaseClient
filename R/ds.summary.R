...
ds.summary <- function(x=NULL, datasources=NULL){
  ...
  if(typ == "data.frame" | typ == "matrix"){
    for(i in 1:numsources){
      ...
      if(validity){
        ...
        r <- dims[[1]][1]
        c <- dims[[1]][2]
        cols <- (datashield.aggregate(datasources[i], as.symbol(paste0('colnames(', x, ')' )), async = FALSE))[[1]]
  ...
  if(typ == "character"){
    for(i in 1:numsources){
      validity <- datashield.aggregate(datasources[i], as.symbol(paste0('isValidDS(', x, ')')))[[1]]
      if(validity){
        l <- datashield.aggregate(datasources[i], as.symbol(paste0('length(', x, ')' )), async = FALSE)[[1]]
  ...
  if(typ == "factor"){
    for(i in 1:numsources){
      ...
      if(validity){
        l <- datashield.aggregate(datasources[i], as.symbol(paste0('length(', x, ')' )), async = FALSE)[[1]]
        categories <- datashield.aggregate(datasources[i], as.symbol(paste0('levels(', x, ')' )), async = FALSE)[[1]]
