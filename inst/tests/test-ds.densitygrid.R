...
grid <- ds.densitygrid(datasources=conns, quote(D$LAB_TSC), quote(D$LAB_HDL), type="combine")
print(grid)
# TODO do more than a smoke test

context("dsbaseclient::ds.densitygrid(type=split)")
grid <- ds.densitygrid(datasources=conns, quote(D$LAB_TSC), quote(D$LAB_HDL), type="split")
print(grid)
# TODO do more than a smoke test

context("dsbaseclient::ds.densitygrid(type=split, numints=15)")
grid <- ds.densitygrid(datasources=conns, quote(D$LAB_TSC), quote(D$LAB_HDL), type="split", numints=15)
print(grid)
# TODO do more than a smoke test
