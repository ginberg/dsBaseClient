...
ds.subset(datasources=conns, subset='subD', data='D', rows=c(1:50), cols=c(1,2))
...
ds.subset(datasources=conns, subset='subD', data='D', rows=c(1:50), cols <- c('DIS_DIAB','PM_BMI_CONTINUOUS'))
# TODO do more than a smoke test

context("dsbaseclient::ds.subset() generate a subset of the table D with bmi values greater than or equal to 25.")
ds.subset(datasources=conns, subset='subD', data='D', logical='PM_BMI_CONTINUOUS>=', threshold=25)
# TODO do more than a smoke test
...
ds.assign(conns, "logHDL", "log(D$LAB_HDL)")
ds.subset(datasources=conns, subset="subLAB_HDL", data="logHDL", rows=c(1:50))
# TODO do more than a smoke test
...
ds.assign(conns, "BMI", "D$PM_BMI_CONTINUOUS")
ds.subset(datasources=conns, subset='subBMI', data='BMI', logical='>=', threshold=25)
# TODO do more than a smoke test
